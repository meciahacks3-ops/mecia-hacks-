...
import { parseProjectTypeFromTeam } from './teamUtils.js';
import { FINAL_ROUND_TEAMS, getFinalRoundTeamInfo } from './finalRoundTeams.js';
...
function getTeamRowData(team, evalEntry, panelInfo, index) {
  ...
  const projectType = parseProjectTypeFromTeam(team);
...
/**
 * Flattens teams into an array of individual students (Leaders & Members) with complete metadata
 */
export function extractAllStudentsRoster(teams = []) {
  const students = [];
  let counter = 1;

  teams.forEach(t => {
    const slot = parseTimeSlotFromTeam(t) || t.timeSlot || 'TBA';
    const rawJudge = (t.assignedJudge || '').trim().toUpperCase();
    const panelProfile = JUDGE_PROFILES[rawJudge];
    const panelLocation = panelProfile?.location || (rawJudge && rawJudge !== 'UNASSIGNED' ? 'Assigned Room' : 'Not Allocated');
    const panelDisplay = panelProfile ? `${panelProfile.id} (${panelProfile.group})` : (t.assignedJudge || 'Unassigned');
    const judgeNames = panelProfile?.namesText || (rawJudge && rawJudge !== 'UNASSIGNED' ? t.assignedJudge : 'Pending Allocation');

    // 1. Team Leader
    students.push({
      sNo: counter++,
      teamId: t.teamIdNo || 'N/A',
      teamName: t.teamName || 'Untitled Team',
      projectTitle: t.projectTitle || 'N/A',
      role: 'Team Leader',
      isLeader: true,
      studentName: t.leaderName || 'N/A',
      enrollmentNo: t.leaderId || 'N/A',
      department: t.leaderBranch || 'Computer Engineering (CE)',
      phone: t.leaderPhone || 'N/A',
      email: t.leaderEmail || 'N/A',
      timeSlot: slot,
      assignedJudge: t.assignedJudge || 'Unassigned',
      judgeNames,
      panelDisplay,
      panelLocation,
      attendanceStatus: '',
    ...
    rawMembers.forEach(m => {
      ...
      const cleanId = (m.idNo || '').trim().toLowerCase();
    ...
    uniqueMembers.forEach((m) => {
      students.push({
        ...
        role: 'Member',
        isLeader: false,
        studentName: m.name || 'N/A',
        enrollmentNo: m.idNo || 'N/A',
        department: m.branch || t.leaderBranch || 'Computer Engineering (CE)',
        phone: m.phone || 'N/A',
        email: m.email || 'N/A',
        timeSlot: slot,
        assignedJudge: t.assignedJudge || 'Unassigned',
...
/**
 * Round-2 Judge Panel IDs (JM001 to JM010)
 */
export const ROUND_2_PANEL_IDS = [
  'JM001', 'JM002', 'JM003', 'JM004', 'JM005',
  'JM006', 'JM007', 'JM008', 'JM009', 'JM010'
];

/**
 * Generates and downloads a dedicated Excel Attendance Workbook (.xlsx)
 * Organized Panel-Wise strictly for Panels 001 to 010 (JM001 - JM010)
 */
export function exportAttendanceSheetExcel(teams = []) {
  const wb = XLSX.utils.book_new();
  const dateStr = new Date().toLocaleString();
  ...
  const colWidths = [
    ...
    { wch: 20 }, // Judge Panel
    { wch: 26 }, // Lab Location
    { wch: 32 }  // Student Signature
  ];

  // 1. MASTER PANEL-WISE ATTENDANCE SHEET (JM001 to JM010)
  const masterRows = [
    ['MECIA HACK 3.0 — OFFICIAL STUDENT ATTENDANCE SHEET (PANELS JM001 TO JM010)'],
    ['Event:', 'Round 2 Prototype Demo & Final Evaluation', 'Date:', '31st August 2026', 'Generated On:', dateStr],
    ['Total Registered Teams:', teams.length, 'Total Student Participants:', allStudents.length],
    [],
    attendanceHeaders
  ];

  let masterCounter = 1;
  ROUND_2_PANEL_IDS.forEach(panelId => {
    const panelProfile = JUDGE_PROFILES[panelId];
    const panelStudents = allStudents.filter(s => (s.assignedJudge || '').trim().toUpperCase() === panelId);

    // Sort by time slot then team ID then leader first
    panelStudents.sort((a, b) => {
      const slotOrder = { '09:30 AM - 11:30 AM': 1, '12:15 PM - 02:15 PM': 2, '02:30 PM - 04:15 PM': 3, 'TBA': 4 };
    ...
    panelStudents.forEach(s => {
      masterRows.push([
        ...
        s.department,
        s.timeSlot,
        `${panelId} (${panelProfile?.group || ''})`,
        panelProfile?.location || s.panelLocation,
        '' // Student Signature blank for physical sign
      ]);
    });
  });
  ...
  XLSX.utils.book_append_sheet(wb, wsMaster, 'All Panels (001-010)');

  // 2. DEDICATED INDIVIDUAL SHEETS FOR EACH PANEL (JM001 to JM010)
  ROUND_2_PANEL_IDS.forEach(panelId => {
    const panelProfile = JUDGE_PROFILES[panelId];
    const panelStudents = allStudents.filter(s => (s.assignedJudge || '').trim().toUpperCase() === panelId);

    // Sort by time slot then team ID
    panelStudents.sort((a, b) => {
      const slotOrder = { '09:30 AM - 11:30 AM': 1, '12:15 PM - 02:15 PM': 2, '02:30 PM - 04:15 PM': 3, 'TBA': 4 };
      const slotA = slotOrder[a.timeSlot] || 5;
      const slotB = slotOrder[b.timeSlot] || 5;
    ...
    const pRows = [
      [`MECIA HACK 3.0 — LAB ATTENDANCE SHEET: ${panelId} (${panelProfile?.group || ''})`],
      [`Lab Venue / Location: ${panelProfile?.location || 'N/A'}`, `Faculty Evaluators: ${panelProfile?.namesText || 'N/A'}`],
      [`Total Students in Lab: ${panelStudents.length}`, `Date: 31st August 2026`],
      [],
      attendanceHeaders
    ];

    panelStudents.forEach((s, idx) => {
      pRows.push([
        idx + 1,
        s.teamId,
        s.teamName,
        ...
        panelId,
        panelProfile?.location || s.panelLocation,
        '' // Student Signature blank
      ]);
    });

    const wsPanel = XLSX.utils.aoa_to_sheet(pRows);
    wsPanel['!cols'] = colWidths;
    const roomCode = panelProfile?.location.split(',')[1]?.trim() || panelProfile?.group || panelId;
    const sheetTitle = sanitizeSheetName(`${panelId}_${roomCode}`);
    XLSX.utils.book_append_sheet(wb, wsPanel, sheetTitle);
  });

  // 3. Download Workbook
  const filename = `Mecia_Hack_3.0_Attendance_Panels_JM001_to_JM010_${Date.now()}.xlsx`;
  XLSX.writeFile(wb, filename);
  return filename;
}

/**
 * Opens a print dialog with a formatted A4 Attendance Sheet
 * Generates distinct pages for Panels 001 to 010 (JM001 - JM010) with page breaks
 * Excludes Present/Absent and Remarks columns and provides clean signature column
 */
export function printAttendanceSheet(teams = [], options = {}) {
  if (typeof window === 'undefined') return;

  const filterPanelId = options.panelId ? options.panelId.trim().toUpperCase() : null;
  const dateStr = new Date().toLocaleString();
  const allStudents = extractAllStudentsRoster(teams);

  const printWindow = window.open('', '_blank', 'width=1150,height=850');
  if (!printWindow) {
    alert('Please allow popups to open the printable attendance sheet!');
    return;
  }

  // Target panels: JM001 to JM010 (or specific panel if selected)
  const targetPanelIds = filterPanelId
    ? [filterPanelId]
    : ROUND_2_PANEL_IDS;

  const panelsHtml = targetPanelIds.map((panelId, pIdx) => {
    const panelProfile = JUDGE_PROFILES[panelId] || {
      id: panelId,
      group: `Group ${panelId.replace('JM', '')}`,
      namesText: 'Assigned Judges',
      location: 'Assigned Lab Room'
    };

    const panelStudents = allStudents.filter(s => (s.assignedJudge || '').trim().toUpperCase() === panelId);

    // Sort by time slot then Team ID then leader first
    const sortedStudents = [...panelStudents].sort((a, b) => {
      const slotOrder = { '09:30 AM - 11:30 AM': 1, '12:15 PM - 02:15 PM': 2, '02:30 PM - 04:15 PM': 3, 'TBA': 4 };
      const slotA = slotOrder[a.timeSlot] || 5;
      const slotB = slotOrder[b.timeSlot] || 5;
      if (slotA !== slotB) return slotA - slotB;
    ...
    const uniqueTeamsCount = new Set(sortedStudents.map(s => s.teamId)).size;

    const rowsHtml = sortedStudents.length === 0
      ? `<tr><td colspan="8" style="text-align:center; padding: 25px; font-style: italic; color: #555;">No student participants assigned to this judge panel yet.</td></tr>`
      : sortedStudents.map((s, idx) => `
          <tr style="${s.isLeader ? 'background-color: #fdfdf0;' : ''}">
            ...
            <td style="font-weight: bold; font-family: monospace; font-size: 11px;">
              ${s.teamId}<br/>
              <span style="font-family: Arial, sans-serif; font-size: 11px; font-weight: 600; color: #111;">${s.teamName}</span>
            </td>
            <td style="font-size: 11px; font-weight: 600; ${s.isLeader ? 'color: #b8860b;' : 'color: #333;'}">
              ${s.role}
            </td>
            <td style="font-weight: bold; font-size: 12px;">
              ${s.studentName}
            </td>
            <td style="text-align: center; font-family: monospace; font-size: 12px; font-weight: bold; background: #fafafa;">
              ${s.enrollmentNo}
            </td>
            <td style="font-size: 11px; font-weight: 500;">
              ${s.department}
            </td>
            <td style="text-align: center; font-size: 10.5px; font-family: monospace; font-weight: bold;">
              ${s.timeSlot}
            </td>
            <td style="width: 170px; text-align: center; vertical-align: bottom; padding-bottom: 8px;">
              <div style="border-bottom: 1.5px solid #222; width: 92%; margin: 0 auto; height: 22px;"></div>
              <span style="font-size: 8.5px; color: #777;">Student Signature</span>
            </td>
          </tr>
        `).join('');

    return `
      <div class="page-container" style="${pIdx < targetPanelIds.length - 1 ? 'page-break-after: always;' : ''} margin-bottom: 30px;">
        <div class="header">
          <div style="display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 2px solid #000; padding-bottom: 6px; margin-bottom: 8px;">
            <div>
              <h1 style="margin: 0; font-size: 17px; text-transform: uppercase; letter-spacing: 0.5px;">MECIA HACK 3.0 — OFFICIAL PARTICIPANT ATTENDANCE SHEET</h1>
              <div style="font-size: 12px; font-weight: bold; color: #222; margin-top: 2px;">ROUND 2 EVALUATION & VERIFICATION DOSSIER</div>
            </div>
            <div style="text-align: right; font-size: 11px;">
              <div><strong>Date:</strong> 31st August 2026</div>
              <div><strong>Generated:</strong> ${dateStr}</div>
            </div>
          </div>

          <div style="display: grid; grid-template-columns: 1.5fr 1.5fr 1fr; gap: 10px; background: #f4f4f4; padding: 8px 12px; border: 1px solid #ccc; font-size: 12px;">
            <div><strong>🏛️ JUDGE PANEL:</strong> ${panelProfile.id} (${panelProfile.group})</div>
            <div><strong>📍 LAB VENUE / ROOM:</strong> <span style="font-weight: bold; background: #e0f2fe; padding: 2px 6px; border: 1px solid #93c5fd; border-radius: 3px;">${panelProfile.location}</span></div>
            <div style="text-align: right;"><strong>👥 TEAMS:</strong> ${uniqueTeamsCount} | <strong>STUDENTS:</strong> ${sortedStudents.length}</div>
          </div>
          
          <div style="margin-top: 6px; font-size: 11px; padding: 4px 8px; background: #fff; border: 1px dashed #bbb;">
            <strong>👨‍🏫 FACULTY EVALUATORS:</strong> ${panelProfile.namesText}
          </div>
        </div>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th style="width: 150px;">Team ID & Name</th>
              <th style="width: 110px;">Role</th>
              <th style="width: 160px;">Student Name</th>
              <th style="width: 130px;">Enrollment / ID</th>
              <th style="width: 140px;">Department / Branch</th>
              <th style="width: 120px;">Time Slot</th>
              <th style="width: 170px;">Student Signature</th>
            </tr>
          </thead>
          <tbody>
            ${rowsHtml}
        ...
        <div style="margin-top: 24px; display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px; font-size: 11px; page-break-inside: avoid;">
          <div style="border-top: 1.5px solid #000; padding-top: 6px; text-align: center;">
            <strong>Student Volunteer / Lab Incharge</strong><br/>
            <span style="color: #666; font-size: 9.5px;">(Signature & Name)</span>
          </div>
          <div style="border-top: 1.5px solid #000; padding-top: 6px; text-align: center;">
            <strong>Faculty Judge / Evaluator</strong><br/>
            <span style="color: #666; font-size: 9.5px;">(Signature & Verification)</span>
          </div>
          <div style="border-top: 1.5px solid #000; padding-top: 6px; text-align: center;">
            <strong>Mecia Hack Convener / Admin</strong><br/>
            <span style="color: #666; font-size: 9.5px;">(Official Stamp & Sign)</span>
          </div>
        </div>
      </div>
    `;
  ...
  const html = `
    ...
    <html>
    <head>
      <title>Mecia Hack 3.0 — Judge Panels Attendance Sheets (JM001 - JM010)</title>
      <style>
        @page {
          size: A4 landscape;
          margin: 8mm;
    ...
    <body>
      <div class="no-print" style="margin-bottom: 14px; display: flex; gap: 10px; align-items: center; background: #e0f2fe; padding: 10px 14px; border: 1px solid #93c5fd; border-radius: 6px;">
        <button onclick="window.print()" style="padding: 10px 22px; font-weight: bold; background: #107c41; color: #fff; border: none; border-radius: 4px; cursor: pointer; font-size: 13px;">
          🖨️ PRINT PANEL ATTENDANCE SHEETS (JM001 - JM010)
        </button>
        <button onclick="window.close()" style="padding: 10px 16px; background: #555; color: #fff; border: none; border-radius: 4px; cursor: pointer; font-size: 13px;">
          ✕ Close Window
        </button>
        <span style="font-size: 12px; color: #0369a1; margin-left: 10px;">
          💡 <em>Tip: Each panel (JM001 - JM010) will automatically print on its own dedicated A4 page.</em>
        </span>
      </div>
...
/**
 * Exports CSV file organized Panel-Wise for Panels JM001 to JM010 without P/A or Remarks
 */
export function exportAttendanceCSV(teams = []) {
  const allStudents = extractAllStudentsRoster(teams);
  const csvRows = [];

  csvRows.push(['Mecia Hack 3.0 — Official Student Attendance & Verification Sheet (Panels JM001 to JM010)']);
  csvRows.push(['Generated On', new Date().toLocaleString()]);
  csvRows.push(['Total Registered Teams', teams.length]);
  csvRows.push(['Total Student Participants', allStudents.length]);
  csvRows.push([]);
  ...
  const downloadAnchor = document.createElement('a');
  downloadAnchor.href = url;
  downloadAnchor.setAttribute('download', `Mecia_Hack_3.0_Attendance_Panels_JM001_to_JM010_${Date.now()}.csv`);
  document.body.appendChild(downloadAnchor);
  downloadAnchor.click();
  document.body.removeChild(downloadAnchor);
}
...
// =========================================================================
// PROJECT TRACKS EXCEL EXPORTS (HYBRID, SOFTWARE, HARDWARE SEPARATELY)
// =========================================================================

/**
 * Builds a dedicated worksheet for a specific project track (Software, Hybrid, Hardware, or Master)
 */
function buildTrackSheetRows(trackTitle, trackType, trackTeams = [], evaluations = [], dateStr = '') {
  let scoredCount = 0;
  let pendingCount = 0;
  let totalScoreSum = 0;
  let highestScore = 0;
  ...
  // Process & enrich team data with evaluation scores
  const enrichedTeams = trackTeams.map(t => {
    const evalEntry = evaluations.find(e => (e.teamName || '').toLowerCase() === (t.teamName || '').toLowerCase());
    let isScored = false;
    let score = 0;
    ...
    const pType = parseProjectTypeFromTeam(t);
  ...
  // Sort: Scored teams ranked by highest score first, then pending teams by Team ID
  ...
  const avgScore = scoredCount > 0 ? (totalScoreSum / scoredCount).toFixed(1) : '0.0';

  const trackHeaderName = trackType === 'All' ? 'ALL PROJECT TRACKS MASTER ROSTER' : `${trackType.toUpperCase()} TRACK EVALUATION ROSTER`;
  ...
  const sheetRows = [
    [`MECIA HACK 3.0 — ${trackHeaderName}`],
    [
      `TRACK: ${trackType.toUpperCase()}`,
      `TOTAL TEAMS: ${trackTeams.length}`,
      `EVALUATED: ${scoredCount}`,
      `PENDING: ${pendingCount}`,
      `AVG SCORE: ${avgScore} / 50`,
      `TOP SCORE: ${highestScore} / 50`
    ],
    [
      `TIME SLOTS: Slot 1 (09:30-11:30): ${s1Count}`,
      `Slot 2 (12:15-02:15): ${s2Count}`,
      `Slot 3 (02:30-04:15): ${s3Count}`,
      `TBA: ${tbaCount}`,
      `REPORT GENERATED: ${dateStr || new Date().toLocaleString()}`
    ],
    [],
    ['--- EVALUATION CRITERIA REFERENCE (MAX 10 MARKS EACH | TOTAL: 50 MARKS) ---'],
    ['C1: System Architecture | C2: Prototype Scope | C3: Component Availability | C4: Execution Feasibility | C5: Implementation Details'],
    [],
    headers
  ];

  if (sortedTeams.length === 0) {
    sheetRows.push(['-', 'N/A', `(No ${trackType} teams found)`, trackType, '-', '-', 'TBA', 'Unassigned', '-', '-', '-', '-', '-', '-', '-', '-', '', '', '', '', '', '', 'PENDING', '']);
  } else {
    let rankIndex = 1;
    sortedTeams.forEach((t, idx) => {
      let rankBadge = '-';
      if (t.isScored) {
        if (rankIndex === 1) rankBadge = '🥇 1ST';
        else if (rankIndex === 2) rankBadge = '🥈 2ND';
        else if (rankIndex === 3) rankBadge = '🥉 3RD';
        else rankBadge = `#${rankIndex}`;
        rankIndex++;
      }

      const m1Str = t.m1 ? `${t.m1.name || ''} (${t.m1.idNo || ''})${t.m1.branch ? ` [${t.m1.branch}]` : ''}` : '-';
      const m2Str = t.m2 ? `${t.m2.name || ''} (${t.m2.idNo || ''})${t.m2.branch ? ` [${t.m2.branch}]` : ''}` : '-';
      const m3Str = t.m3 ? `${t.m3.name || ''} (${t.m3.idNo || ''})${t.m3.branch ? ` [${t.m3.branch}]` : ''}` : '-';

      sheetRows.push([
        rankBadge !== '-' ? rankBadge : `Pending (${idx + 1})`,
        t.teamIdNo || 'N/A',
        t.teamName || '',
        t.projectType || trackType,
  ...
  ws['!cols'] = [
    ...
    { wch: 12 }, // Team ID
    { wch: 24 }, // Team Name
    { wch: 15 }, // Project Track
    { wch: 30 }, // Project Title
    { wch: 22 }, // Tech Stack
    { wch: 24 }, // Time Slot
    { wch: 22 }, // Assigned Judge
    { wch: 22 }, // Leader Name
    { wch: 18 }, // Leader ID
    { wch: 16 }, // Leader Phone
    { wch: 26 }, // Leader Email
...
/**
 * Builds the Summary and Cross-Track Comparison sheet
 */
function buildTracksSummarySheetRows(teams = [], evaluations = [], dateStr = '') {
  const softwareTeams = teams.filter(t => parseProjectTypeFromTeam(t).toLowerCase() === 'software');
  const hybridTeams = teams.filter(t => parseProjectTypeFromTeam(t).toLowerCase() === 'hybrid');
  const hardwareTeams = teams.filter(t => parseProjectTypeFromTeam(t).toLowerCase() === 'hardware');

  const calcTrackStats = (trackList) => {
    let scored = 0;
    let totalScore = 0;
    let highest = 0;
    const scoredTeams = [];
    ...
    trackList.forEach(t => {
      ...
      if (evalEntry) {
        scored++;
        const s = Number(evalEntry.totalScore) || 0;
        totalScore += s;
        if (s > highest) highest = s;
        scoredTeams.push({ team: t, score: s, evalEntry });
      }
    });

    scoredTeams.sort((a, b) => b.score - a.score);

    const avg = scored > 0 ? (totalScore / scored).toFixed(1) : '0.0';
    return {
      total: trackList.length,
      scored,
      pending: trackList.length - scored,
      avg,
      highest,
      top1: scoredTeams[0] ? `${scoredTeams[0].team.teamName} (${scoredTeams[0].team.teamIdNo || 'N/A'}) — ${scoredTeams[0].score}/50` : 'None',
      top2: scoredTeams[1] ? `${scoredTeams[1].team.teamName} (${scoredTeams[1].team.teamIdNo || 'N/A'}) — ${scoredTeams[1].score}/50` : 'None',
      top3: scoredTeams[2] ? `${scoredTeams[2].team.teamName} (${scoredTeams[2].team.teamIdNo || 'N/A'}) — ${scoredTeams[2].score}/50` : 'None',
      top5List: scoredTeams.slice(0, 5)
    };
  };

  const softStats = calcTrackStats(softwareTeams);
  const hybStats = calcTrackStats(hybridTeams);
  const hardStats = calcTrackStats(hardwareTeams);
  const totalStats = calcTrackStats(teams);

  const sheetRows = [
    ['MECIA HACK 3.0 — PROJECT TRACKS SUMMARY & EXECUTIVE LEADERBOARD'],
    [`GENERATED ON: ${dateStr || new Date().toLocaleString()}`, `TOTAL TEAMS: ${teams.length}`, `TOTAL EVALUATED: ${totalStats.scored}`],
    [],
    ['=== TRACK-WISE BREAKDOWN & METRICS ==='],
    ['Track', 'Icon', 'Total Teams', 'Scored Teams', 'Pending Teams', 'Completion %', 'Avg Score (50)', 'Highest Score (50)', 'Rank #1 Team (Gold)', 'Rank #2 Team (Silver)', 'Rank #3 Team (Bronze)'],
    [
      'Software Track',
      '💻',
      softStats.total,
      softStats.scored,
      softStats.pending,
      softStats.total > 0 ? `${Math.round((softStats.scored / softStats.total) * 100)}%` : '0%',
      softStats.avg,
      softStats.highest,
      softStats.top1,
      softStats.top2,
    ...
    [
      ...
      hybStats.scored,
      hybStats.pending,
      hybStats.total > 0 ? `${Math.round((hybStats.scored / hybStats.total) * 100)}%` : '0%',
      hybStats.avg,
      hybStats.highest,
      hybStats.top1,
      hybStats.top2,
    ...
    [
      ...
      hardStats.scored,
      hardStats.pending,
      hardStats.total > 0 ? `${Math.round((hardStats.scored / hardStats.total) * 100)}%` : '0%',
      hardStats.avg,
      hardStats.highest,
      hardStats.top1,
      hardStats.top2,
    ...
    [
      'OVERALL HACKATHON TOTAL',
      '🏆',
      totalStats.total,
      totalStats.scored,
      totalStats.pending,
      totalStats.total > 0 ? `${Math.round((totalStats.scored / totalStats.total) * 100)}%` : '0%',
      totalStats.avg,
      totalStats.highest,
      totalStats.top1,
      totalStats.top2,
    ...
    [],
    ['=== TOP 5 LEADERBOARDS BY PROJECT TRACK ==='],
    []
  ];

  // Top 5 Software
  sheetRows.push(['--- 💻 TOP 5 SOFTWARE PROJECTS ---']);
  sheetRows.push(['Rank', 'Team ID', 'Team Name', 'Score (50)', 'Project Title', 'Leader', 'Assigned Judge']);
  if (softStats.top5List.length === 0) {
    sheetRows.push(['-', '-', 'No evaluated software teams yet', '-', '-', '-', '-']);
  } else {
    softStats.top5List.forEach((item, idx) => {
      sheetRows.push([
        idx === 0 ? '🥇 1ST' : idx === 1 ? '🥈 2ND' : idx === 2 ? '🥉 3RD' : `#${idx + 1}`,
        item.team.teamIdNo || 'N/A',
        item.team.teamName,
        item.score,
        item.team.projectTitle || 'Untitled',
        item.team.leaderName || '',
        item.team.assignedJudge || 'Unassigned'
      ]);
    });
  }
  ...
  sheetRows.push([]);
  // Top 5 Hybrid
  sheetRows.push(['--- ⚡ TOP 5 HYBRID PROJECTS ---']);
  sheetRows.push(['Rank', 'Team ID', 'Team Name', 'Score (50)', 'Project Title', 'Leader', 'Assigned Judge']);
  if (hybStats.top5List.length === 0) {
    sheetRows.push(['-', '-', 'No evaluated hybrid teams yet', '-', '-', '-', '-']);
  } else {
    hybStats.top5List.forEach((item, idx) => {
      sheetRows.push([
        idx === 0 ? '🥇 1ST' : idx === 1 ? '🥈 2ND' : idx === 2 ? '🥉 3RD' : `#${idx + 1}`,
        item.team.teamIdNo || 'N/A',
        item.team.teamName,
  ...
  sheetRows.push([]);
  // Top 5 Hardware
  sheetRows.push(['--- ⚙️ TOP 5 HARDWARE PROJECTS ---']);
  sheetRows.push(['Rank', 'Team ID', 'Team Name', 'Score (50)', 'Project Title', 'Leader', 'Assigned Judge']);
  if (hardStats.top5List.length === 0) {
    sheetRows.push(['-', '-', 'No evaluated hardware teams yet', '-', '-', '-', '-']);
  } else {
    hardStats.top5List.forEach((item, idx) => {
      sheetRows.push([
        idx === 0 ? '🥇 1ST' : idx === 1 ? '🥈 2ND' : idx === 2 ? '🥉 3RD' : `#${idx + 1}`,
        item.team.teamIdNo || 'N/A',
        item.team.teamName,
...
/**
 * Creates and downloads the Master Multi-Sheet Excel Workbook separated by Software, Hybrid, and Hardware tracks
 */
export function exportProjectTracksWorkbook(teams = [], evaluations = []) {
  const wb = XLSX.utils.book_new();
  const dateStr = new Date().toLocaleString();
  ...
  // Sheet 1: Track Summary & Analytics
  const wsSummary = buildTracksSummarySheetRows(teams, evaluations, dateStr);
  XLSX.utils.book_append_sheet(wb, wsSummary, 'Track Summary & Stats');

  // Sheet 2: Software Track
  const wsSoftware = buildTrackSheetRows('Software Track', 'Software', softwareTeams, evaluations, dateStr);
  XLSX.utils.book_append_sheet(wb, wsSoftware, sanitizeSheetName(`Software (${softwareTeams.length} Teams)`));

  // Sheet 3: Hybrid Track
  const wsHybrid = buildTrackSheetRows('Hybrid Track', 'Hybrid', hybridTeams, evaluations, dateStr);
  XLSX.utils.book_append_sheet(wb, wsHybrid, sanitizeSheetName(`Hybrid (${hybridTeams.length} Teams)`));

  // Sheet 4: Hardware Track
  const wsHardware = buildTrackSheetRows('Hardware Track', 'Hardware', hardwareTeams, evaluations, dateStr);
  XLSX.utils.book_append_sheet(wb, wsHardware, sanitizeSheetName(`Hardware (${hardwareTeams.length} Teams)`));

  // Sheet 5: Master All Tracks Roster
  const wsAll = buildTrackSheetRows('All Tracks Master Roster', 'All', teams, evaluations, dateStr);
  XLSX.utils.book_append_sheet(wb, wsAll, sanitizeSheetName(`All Tracks (${teams.length} Teams)`));

  const fileName = `Mecia_Hack_3.0_Project_Tracks_Master_Workbook_${Date.now()}.xlsx`;
  XLSX.writeFile(wb, fileName);
  return fileName;
}

/**
 * Creates and downloads a single standalone Excel workbook for one specific track (Software, Hybrid, or Hardware)
 */
export function exportSingleTrackExcel(trackType = 'software', teams = [], evaluations = []) {
  const normType = trackType.trim().toLowerCase();
  const matchedTeams = teams.filter(t => {
    if (normType === 'all') return true;
    return parseProjectTypeFromTeam(t).toLowerCase() === normType;
  });

  const displayType = normType === 'software' ? 'Software' : normType === 'hybrid' ? 'Hybrid' : normType === 'hardware' ? 'Hardware' : 'All';
  const wb = XLSX.utils.book_new();
  const dateStr = new Date().toLocaleString();

  const ws = buildTrackSheetRows(`${displayType} Track`, displayType, matchedTeams, evaluations, dateStr);
  XLSX.utils.book_append_sheet(wb, ws, sanitizeSheetName(`${displayType} Track (${matchedTeams.length})`));

  const fileName = `Mecia_Hack_3.0_${displayType}_Track_Sheet_${Date.now()}.xlsx`;
  XLSX.writeFile(wb, fileName);
  return fileName;
}

/**
 * Export all tracks as individual separate .xlsx sheets packaged in a ZIP archive
 */
export async function exportAllTracksZip(teams = [], evaluations = []) {
  const zip = new JSZip();
  const dateStr = new Date().toLocaleString();

  const tracks = [
    { type: 'Software', filter: 'software' },
    { type: 'Hybrid', filter: 'hybrid' },
    { type: 'Hardware', filter: 'hardware' }
  ];

  tracks.forEach(tr => {
    const list = teams.filter(t => parseProjectTypeFromTeam(t).toLowerCase() === tr.filter);
    const wb = XLSX.utils.book_new();
    const ws = buildTrackSheetRows(`${tr.type} Track`, tr.type, list, evaluations, dateStr);
    XLSX.utils.book_append_sheet(wb, ws, `${tr.type} Track`);
    const arrayBuffer = XLSX.write(wb, { type: 'array', bookType: 'xlsx' });
    zip.file(`Mecia_Hack_3.0_${tr.type}_Track_${list.length}_Teams.xlsx`, arrayBuffer);
  });

  // Also include Master Tracks Workbook in zip
  const wbMaster = XLSX.utils.book_new();
  const wsSummary = buildTracksSummarySheetRows(teams, evaluations, dateStr);
  XLSX.utils.book_append_sheet(wbMaster, wsSummary, 'Track Summary');

  tracks.forEach(tr => {
    const list = teams.filter(t => parseProjectTypeFromTeam(t).toLowerCase() === tr.filter);
    const ws = buildTrackSheetRows(`${tr.type} Track`, tr.type, list, evaluations, dateStr);
    XLSX.utils.book_append_sheet(wbMaster, ws, `${tr.type} Track`);
  });

  const masterArrayBuffer = XLSX.write(wbMaster, { type: 'array', bookType: 'xlsx' });
  zip.file(`Mecia_Hack_3.0_ALL_TRACKS_COMBINED_WORKBOOK.xlsx`, masterArrayBuffer);

  const content = await zip.generateAsync({ type: 'blob' });
  const zipFileName = `Mecia_Hack_3.0_Project_Tracks_Separate_Sheets_${Date.now()}.zip`;
...
/**
 * Creates and downloads the Leaderboard Special Selection Workbook:
 * Top 30 Software + Top 15 Hybrid + All Hardware Teams
 */
export function exportTop30Soft15HybAllHardExcel(teams = [], evaluations = []) {
  const wb = XLSX.utils.book_new();
  const dateStr = new Date().toLocaleString();

  // 1. Prepare ranked leaderboard data with scores
  ...
  const softwareTeams = leaderboardData.filter(t => (t.projectType || '').toLowerCase() === 'software');
  const hybridTeams = leaderboardData.filter(t => (t.projectType || '').toLowerCase() === 'hybrid');
  const hardwareTeams = leaderboardData.filter(t => (t.projectType || '').toLowerCase() === 'hardware');

  const top30Software = softwareTeams.slice(0, 30);
  const top15Hybrid = hybridTeams.slice(0, 15);
  const allHardware = hardwareTeams;

  const combinedSelected = [
    ...top30Software.map((t, idx) => ({ ...t, selectionCategory: 'Top 30 Software', categoryRank: idx + 1 })),
    ...top15Hybrid.map((t, idx) => ({ ...t, selectionCategory: 'Top 15 Hybrid', categoryRank: idx + 1 })),
    ...allHardware.map((t, idx) => ({ ...t, selectionCategory: 'All Hardware', categoryRank: idx + 1 }))
  ];

  function buildSpecialTrackSheetRows(sheetTitle, trackLabel, teamList) {
    const rows = [
      [`MECIA HACK 3.0 — ${sheetTitle.toUpperCase()}`],
      [`GENERATED: ${dateStr}`, `TRACK: ${trackLabel}`, `TOTAL TEAMS: ${teamList.length}`, `TOTAL PARTICIPANTS: ${teamList.reduce((s, t) => s + (t.totalTeamSize || (1 + (t.members?.length || 0))), 0)}`],
      [],
      [
        'Rank',
        'Team ID',
    ...
    teamList.forEach((t, idx) => {
      ...
      const allMembersStr = [
        ...
        ...(t.members || []).map((m, mIdx) => `M${mIdx+1}: ${m.name || ''} (${m.idNo || ''}) Ph: ${m.phone || ''}`)
      ].join(' | ');

      const rankStr = t.isScored ? (idx === 0 ? '🥇 1ST' : idx === 1 ? '🥈 2ND' : idx === 2 ? '🥉 3RD' : `#${idx + 1}`) : '-';
  ...
  const summaryRows = [
    ['MECIA HACKS 3.0 — LEADERBOARD SPECIAL SELECTION EXCEL REPORT'],
    ['SELECTION CRITERIA: TOP 30 SOFTWARE + TOP 15 HYBRID + ALL HARDWARE TEAMS'],
    [`GENERATED ON: ${dateStr}`, `TOTAL SELECTED TEAMS: ${combinedSelected.length}`, `TOTAL PARTICIPANTS: ${combinedSelected.reduce((s, t) => s + (t.totalTeamSize || (1 + (t.members?.length || 0))), 0)}`],
    [],
    ['=== TRACK-WISE SELECTION SUMMARY ==='],
    ['Track Selection', 'Quota / Filter', 'Selected Teams', 'Total Members', 'Evaluated Teams', 'Pending Teams', 'Highest Score (50)', 'Avg Score (50)'],
    [
      'Software Track',
      'Top 30 Ranked',
      top30Software.length,
      top30Software.reduce((s, t) => s + (t.totalTeamSize || (1 + (t.members?.length || 0))), 0),
      top30Software.filter(t => t.isScored).length,
      top30Software.filter(t => !t.isScored).length,
      top30Software.find(t => t.isScored)?.score || 0,
      top30Software.filter(t => t.isScored).length > 0 ? (top30Software.filter(t => t.isScored).reduce((a, b) => a + b.score, 0) / top30Software.filter(t => t.isScored).length).toFixed(1) : 0
    ],
    [
      'Hybrid Track',
      'Top 15 Ranked',
      top15Hybrid.length,
      top15Hybrid.reduce((s, t) => s + (t.totalTeamSize || (1 + (t.members?.length || 0))), 0),
      top15Hybrid.filter(t => t.isScored).length,
      top15Hybrid.filter(t => !t.isScored).length,
      top15Hybrid.find(t => t.isScored)?.score || 0,
      top15Hybrid.filter(t => t.isScored).length > 0 ? (top15Hybrid.filter(t => t.isScored).reduce((a, b) => a + b.score, 0) / top15Hybrid.filter(t => t.isScored).length).toFixed(1) : 0
    ],
    [
      'Hardware Track',
      'All Registered Hardware',
      allHardware.length,
      allHardware.reduce((s, t) => s + (t.totalTeamSize || (1 + (t.members?.length || 0))), 0),
      allHardware.filter(t => t.isScored).length,
      allHardware.filter(t => !t.isScored).length,
      allHardware.find(t => t.isScored)?.score || 0,
      allHardware.filter(t => t.isScored).length > 0 ? (allHardware.filter(t => t.isScored).reduce((a, b) => a + b.score, 0) / allHardware.filter(t => t.isScored).length).toFixed(1) : 0
    ],
    [
      'TOTAL COMBINED SELECTION',
      'Top 30 Soft + Top 15 Hyb + All Hard',
      combinedSelected.length,
      combinedSelected.reduce((s, t) => s + (t.totalTeamSize || (1 + (t.members?.length || 0))), 0),
      combinedSelected.filter(t => t.isScored).length,
      combinedSelected.filter(t => !t.isScored).length,
      combinedSelected.find(t => t.isScored)?.score || 0,
      combinedSelected.filter(t => t.isScored).length > 0 ? (combinedSelected.filter(t => t.isScored).reduce((a, b) => a + b.score, 0) / combinedSelected.filter(t => t.isScored).length).toFixed(1) : 0
    ],
    [],
    ['=== COMBINED MASTER QUALIFIED ROSTER (ALL TRACKS) ==='],
    [
      'S.No',
      'Selection Category',
      'Category Rank',
  ...
  combinedSelected.forEach((t, i) => {
    const allMembersStr = [
      `Leader: ${t.leaderName || ''} (${t.leaderId || ''})`,
      ...(t.members || []).map((m, mIdx) => `M${mIdx+1}: ${m.name || ''} (${m.idNo || ''})`)
    ].join('; ');
  ...
  XLSX.utils.book_append_sheet(wb, wsSummary, 'Summary & Combined Roster');

  // Sheet 2: Top 30 Software
  const wsSoft = buildSpecialTrackSheetRows('Top 30 Software Projects', 'Software', top30Software);
  XLSX.utils.book_append_sheet(wb, wsSoft, sanitizeSheetName(`Top 30 Software (${top30Software.length})`));

  // Sheet 3: Top 15 Hybrid
  const wsHyb = buildSpecialTrackSheetRows('Top 15 Hybrid Projects', 'Hybrid', top15Hybrid);
  XLSX.utils.book_append_sheet(wb, wsHyb, sanitizeSheetName(`Top 15 Hybrid (${top15Hybrid.length})`));

  // Sheet 4: All Hardware
  const wsHard = buildSpecialTrackSheetRows('All Hardware Projects', 'Hardware', allHardware);
  XLSX.utils.book_append_sheet(wb, wsHard, sanitizeSheetName(`All Hardware (${allHardware.length})`));

  const fileName = `MECIA_HACKS_3.0_Top30Soft_Top15Hyb_AllHard_Leaderboard_${Date.now()}.xlsx`;
  XLSX.writeFile(wb, fileName);
  return fileName;
}

/**
 * Export Excel spreadsheet with:
 * Team ID, Team Name, Leader Name, and Member Names of finalist teams.
 * Includes dedicated sheets for Clean 4-Column view, Overview with Member 1/2/3,
 * Individual 189 Participants Roster, and Track-wise sheets.
 */
export function exportFinalistTeamsAndMembersExcel(customFinalistTeams = null) {
  const wb = XLSX.utils.book_new();
  const teams = Array.isArray(customFinalistTeams) && customFinalistTeams.length > 0 
    ? customFinalistTeams 
    : FINAL_ROUND_TEAMS;
  ...
  const sheet1Rows = [
    ...
    [
      `TOTAL FINALIST TEAMS: ${teams.length}`,
      `TOTAL PARTICIPANTS: ${teams.reduce((sum, t) => sum + (t.totalMembers || (1 + (t.members?.length || 0))), 0)}`
    ],
    [],
    sheet1Headers
  ];
  ...
  teams.forEach((team, idx) => {
    ...
    sheet1Rows.push([
      idx + 1,
      team.teamId || team.teamIdNo || '',
      team.teamName || '',
      team.track || team.projectType || '',
      team.leaderName || '',
      memberNamesList,
      m1,
      m2,
  ...
  // Sheet 2: Clean Team & Members (Direct 4-column format)
  const sheet2Headers = [
    'Team ID',
    'Team Name',
    'Leader Name',
  ...
  // Sheet 3: Individual Participants Roster
  const sheet3Headers = [
    'S.No',
    'Team ID',
    'Team Name',
  ...
  const sheet3Rows = [
    ['MECIA HACKS 3.0 — ALL FINALIST PARTICIPANTS ROSTER'],
    [],
    sheet3Headers
  ];
  ...
  XLSX.utils.book_append_sheet(wb, ws3, 'All Participants Roster');

  // Sheet 4, 5, 6: Track-wise sheets
  function buildTrackSheet(trackName, filterFn) {
    const trackTeams = teams.filter(filterFn);
    const rows = [
      [`MECIA HACKS 3.0 — ${trackName.toUpperCase()} FINALISTS (${trackTeams.length} TEAMS)`],
      [],
      [
        'S.No',
        'Team ID',
  ...
  const wsSoft = buildTrackSheet('Software Track', t => (t.track || t.projectType || '').toLowerCase() === 'software');
  XLSX.utils.book_append_sheet(wb, wsSoft, 'Software Track (30)');

  const wsHyb = buildTrackSheet('Hybrid Track', t => (t.track || t.projectType || '').toLowerCase() === 'hybrid');
  XLSX.utils.book_append_sheet(wb, wsHyb, 'Hybrid Track (15)');

  const wsHard = buildTrackSheet('Hardware Track', t => (t.track || t.projectType || '').toLowerCase() === 'hardware');
  XLSX.utils.book_append_sheet(wb, wsHard, 'Hardware Track (4)');

  const fileName = `MECIA_HACKS_3.0_Finalist_Teams_Members_${Date.now()}.xlsx`;
  XLSX.writeFile(wb, fileName);
  return fileName;
}

/**
 * Export CSV spreadsheet with:
 * Team ID, Team Name, Leader Name, and Member Names of finalist teams.
 */
export function exportFinalistTeamsAndMembersCSV(customFinalistTeams = null) {
  const teams = Array.isArray(customFinalistTeams) && customFinalistTeams.length > 0 
    ? customFinalistTeams 
    : FINAL_ROUND_TEAMS;
  ...
  teams.forEach((team, idx) => {
    ...
    csvRows.push([
      idx + 1,
      `"${(team.teamId || team.teamIdNo || '').replace(/"/g, '""')}"`,
      `"${(team.teamName || '').replace(/"/g, '""')}"`,
      `"${(team.track || team.projectType || '').replace(/"/g, '""')}"`,
  ...
  const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
  const fileName = `MECIA_HACKS_3.0_Finalist_Teams_Members_${Date.now()}.csv`;
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = fileName;
 ...
 */
export function exportTeamLabLocationAndJudgesCSV(customTeams = null, evaluations = []) {
  const teams = Array.isArray(customTeams) && customTeams.length > 0 
    ? customTeams 
    : FINAL_ROUND_TEAMS;

  const evalsList = Array.isArray(evaluations) ? evaluations : [];
  ...
  const csvRows = [
    [
      ...
      '"Team Name"',
      '"Lab Location"',
      '"Assigned External Panel ID"',
      '"External Jury Names"',
      '"External Panel Group"',
      '"Track"',
      '"Leader Name"',
      '"Leader Phone"',
      '"Project Title"',
      '"Round 2 Rank"',
      '"Round 2 Total Score (50)"',
      '"R2 C1 Innovation (10)"',
      '"R2 C2 Execution (10)"',
      '"R2 C3 Feasibility (10)"',
      '"R2 C4 Presentation (10)"',
      '"R2 C5 Implementation (10)"',
      '"Stage 2 Judge Panel"',
      '"Mentor Panel ID"',
      '"Mentor Names"',
      '"Mentor Phase 1 Feedback"',
      '"Mentor Phase 2 Feedback"'
    ]
  ];

  teams.forEach((team, idx) => {
    const rawJudge = String(team.assignedJudge || '').trim().toUpperCase();
    const isAssigned = rawJudge && rawJudge !== 'UNASSIGNED';
    const profile = isAssigned ? JUDGE_PROFILES[rawJudge] : null;

    const finInfo = team.finalistInfo || getFinalRoundTeamInfo(team);
    const teamId = team.teamIdNo || team.teamId || finInfo?.teamId || 'N/A';
    const teamName = team.teamName || '';
    const labLocation = finInfo?.labLocation || team.labLocation || 'N/A';
    const panelId = isAssigned ? rawJudge : 'Unassigned';
    const panelGroup = profile ? profile.group : (isAssigned ? rawJudge : 'Unassigned');
    const judgesName = profile ? (profile.namesText || profile.names?.join(', ')) : (isAssigned ? rawJudge : 'Unassigned');
    const track = team.projectType || team.track || finInfo?.track || 'N/A';
    const leaderName = team.leaderName || '';
    const leaderPhone = team.leaderPhone || '';
    const projectTitle = team.projectTitle || '';

    // Match Round 2 Marks
    const round2Eval = evalsList.find(e => {
      const nameMatch = (e.teamName || '').trim().toLowerCase() === teamName.trim().toLowerCase();
      const projMatch = projectTitle && projectTitle !== 'Untitled Project' && projectTitle !== 'N/A' && (e.teamName || '').trim().toLowerCase() === projectTitle.trim().toLowerCase();
      const jEmail = (e.judgeEmail || '').trim().toUpperCase();
      return (nameMatch || projMatch) && jEmail.startsWith('JM');
    });

    const r2Rank = finInfo?.rank || 'N/A';
    const r2Score = round2Eval ? round2Eval.totalScore : (finInfo?.score ?? 'N/A');
    const r2C1 = round2Eval ? round2Eval.c1 : (finInfo?.c1 ?? 'N/A');
    const r2C2 = round2Eval ? round2Eval.c2 : (finInfo?.c2 ?? 'N/A');
    const r2C3 = round2Eval ? round2Eval.c3 : (finInfo?.c3 ?? 'N/A');
    const r2C4 = round2Eval ? round2Eval.c4 : (finInfo?.c4 ?? 'N/A');
    const r2C5 = round2Eval ? round2Eval.c5 : (finInfo?.c5 ?? 'N/A');
    const r2Judge = round2Eval ? round2Eval.judgeEmail : (finInfo?.assignedJudge || 'N/A');

    // Match Mentor Evaluation (MM001 - MM010)
    const mentorEval = evalsList.find(e => {
      const nameMatch = (e.teamName || '').trim().toLowerCase() === teamName.trim().toLowerCase();
      const projMatch = projectTitle && projectTitle !== 'Untitled Project' && projectTitle !== 'N/A' && (e.teamName || '').trim().toLowerCase() === projectTitle.trim().toLowerCase();
      const jEmail = (e.judgeEmail || '').trim().toUpperCase();
      return (nameMatch || projMatch) && jEmail.startsWith('MM');
    });

    const mentorPanelId = mentorEval ? mentorEval.judgeEmail : (rawJudge.startsWith('MM') ? rawJudge : 'N/A');
    const mentorProfile = mentorPanelId && mentorPanelId !== 'N/A' ? JUDGE_PROFILES[mentorPanelId] : null;
    const mentorNames = mentorProfile ? (mentorProfile.namesText || mentorProfile.names?.join(', ')) : (mentorPanelId !== 'N/A' ? mentorPanelId : 'N/A');
    const phase1Fb = mentorEval?.phase1Feedback || (mentorEval?.remarks && !mentorEval.hasPhases ? mentorEval.remarks : 'N/A');
    const phase2Fb = mentorEval?.phase2Feedback || 'N/A';

    csvRows.push([
      idx + 1,
      `"${String(teamId).replace(/"/g, '""')}"`,
      `"${String(teamName).replace(/"/g, '""')}"`,
      `"${String(labLocation).replace(/"/g, '""')}"`,
      `"${String(panelId).replace(/"/g, '""')}"`,
      `"${String(judgesName).replace(/"/g, '""')}"`,
      `"${String(panelGroup).replace(/"/g, '""')}"`,
      `"${String(track).replace(/"/g, '""')}"`,
      `"${String(leaderName).replace(/"/g, '""')}"`,
      `"${String(leaderPhone).replace(/"/g, '""')}"`,
      `"${String(projectTitle).replace(/"/g, '""')}"`,
      `"${String(r2Rank).replace(/"/g, '""')}"`,
      `"${String(r2Score).replace(/"/g, '""')}"`,
      `"${String(r2C1).replace(/"/g, '""')}"`,
      `"${String(r2C2).replace(/"/g, '""')}"`,
      `"${String(r2C3).replace(/"/g, '""')}"`,
      `"${String(r2C4).replace(/"/g, '""')}"`,
      `"${String(r2C5).replace(/"/g, '""')}"`,
      `"${String(r2Judge).replace(/"/g, '""')}"`,
      `"${String(mentorPanelId).replace(/"/g, '""')}"`,
      `"${String(mentorNames).replace(/"/g, '""')}"`,
      `"${String(phase1Fb).replace(/"/g, '""').replace(/[\r\n]+/g, ' ')}"`,
      `"${String(phase2Fb).replace(/"/g, '""').replace(/[\r\n]+/g, ' ')}"`
    ]);
  });

  const csvContent = csvRows.map(r => r.join(',')).join('\r\n');
  const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
  const fileName = `MECIA_HACKS_3.0_Finalists_Judges_Marks_Feedback_${Date.now()}.csv`;
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = fileName;
...
/**
 * Export Official Final Round Live Leaderboard Workbook (Excel .xlsx)
 * Ranked dynamically based on external judge marks out of 100 with track breakdown.
 */
export function exportFinalRoundLiveLeaderboardExcel(leaderboardData = []) {
  const wb = XLSX.utils.book_new();
  const dateStr = new Date().toLocaleString();

  function buildFinalRoundSheetRows(title, trackLabel, teamList) {
    const scoredCount = teamList.filter(t => t.isScored).length;
    const rows = [
      [`MECIA HACKS 3.0 — GRAND FINALE LIVE LEADERBOARD (${title.toUpperCase()})`],
      [`GENERATED: ${dateStr}`, `TRACK: ${trackLabel}`, `TOTAL TEAMS: ${teamList.length}`, `EVALUATED: ${scoredCount}/${teamList.length}`, `PENDING: ${teamList.length - scoredCount}`],
      [],
      [
        'Rank',
        'Team ID',
    ...
    teamList.forEach((t, idx) => {
      ...
      rows.push([
        ...
        t.teamName || '',
        t.projectType || trackLabel,
        t.finalistInfo?.labLocation || t.labLocation || 'CE Dept.',
        t.isScored ? t.score : 'Pending',
        t.round2Score ?? t.finalistInfo?.score ?? '-',
        t.isScored ? (t.finalScore ?? t.juryScore) : 'Pending',
        t.isScored ? t.c1 : '-',
        t.isScored ? t.c2 : '-',
        t.isScored ? t.c3 : '-',
        t.isScored ? t.c4 : '-',
        t.isScored ? t.c5 : '-',
        t.isScored ? 'SCORED' : 'PENDING',
        t.judge || t.assignedJudge || 'Awaiting Jury',
        t.remarks || '',
        t.projectTitle || '',
        t.totalTeamSize || (1 + (t.members?.length || 0)),
        t.leaderName || '',
  ...
  // 1. Overall Combined Finalist Leaderboard
  const allRows = buildFinalRoundSheetRows('All Finalists', 'Combined (Software + Hybrid + Hardware)', leaderboardData);
  const wsAll = XLSX.utils.aoa_to_sheet(allRows);
  XLSX.utils.book_append_sheet(wb, wsAll, sanitizeSheetName('Live Leaderboard (Overall)'));

  // 2. Software Track
  const swTeams = leaderboardData.filter(t => (t.projectType || '').toLowerCase() === 'software');
  if (swTeams.length > 0) {
    const swRows = buildFinalRoundSheetRows('Software Track', 'Software', swTeams);
    const wsSw = XLSX.utils.aoa_to_sheet(swRows);
    XLSX.utils.book_append_sheet(wb, wsSw, sanitizeSheetName('Software Track'));
  }

  // 3. Hybrid Track
  const hyTeams = leaderboardData.filter(t => (t.projectType || '').toLowerCase() === 'hybrid');
  if (hyTeams.length > 0) {
    const hyRows = buildFinalRoundSheetRows('Hybrid Track', 'Hybrid', hyTeams);
    const wsHy = XLSX.utils.aoa_to_sheet(hyRows);
    XLSX.utils.book_append_sheet(wb, wsHy, sanitizeSheetName('Hybrid Track'));
  }

  // 4. Hardware Track
  const hwTeams = leaderboardData.filter(t => (t.projectType || '').toLowerCase() === 'hardware');
  if (hwTeams.length > 0) {
    const hwRows = buildFinalRoundSheetRows('Hardware Track', 'Hardware', hwTeams);
    const wsHw = XLSX.utils.aoa_to_sheet(hwRows);
    XLSX.utils.book_append_sheet(wb, wsHw, sanitizeSheetName('Hardware Track'));
  }

  const fileName = `MECIA_HACKS_3.0_Final_Round_Live_Leaderboard_${Date.now()}.xlsx`;
  XLSX.writeFile(wb, fileName);
  return fileName;
}

/**
 * Export Official Final Round Live Leaderboard (.CSV)
 */
export function exportFinalRoundLiveLeaderboardCSV(leaderboardData = []) {
  const csvRows = [
    ['"MECIA HACKS 3.0 — FINAL ROUND LIVE LEADERBOARD CSV"'],
  ...
  leaderboardData.forEach((t, idx) => {
    ...
    csvRows.push([
      ...
      `"${(t.finalistInfo?.labLocation || t.labLocation || 'CE Dept.').replace(/"/g, '""')}"`,
      `"${t.isScored ? t.score : 'Pending'}"`,
      `"${t.round2Score ?? t.finalistInfo?.score ?? '-'}"`,
      `"${t.isScored ? (t.finalScore ?? t.juryScore) : 'Pending'}"`,
      `"${t.isScored ? t.c1 : '-'}"`,
      ...
      `"${t.isScored ? t.c5 : '-'}"`,
      `"${t.isScored ? 'SCORED' : 'PENDING'}"`,
      `"${(t.judge || t.assignedJudge || 'Awaiting Jury').replace(/"/g, '""')}"`,
      `"${(t.remarks || '').replace(/"/g, '""').replace(/[\r\n]+/g, ' ')}"`,
      `"${(t.projectTitle || '').replace(/"/g, '""')}"`,
      t.totalTeamSize || (1 + (t.members?.length || 0)),
      `"${(t.leaderName || '').replace(/"/g, '""')}"`,
      `"${(t.leaderId || '').replace(/"/g, '""')}"`,
      `"${(t.leaderPhone || '').replace(/"/g, '""')}"`,
      `"${(t.leaderEmail || '').replace(/"/g, '""')}"`
    ]);
  });

  const csvContent = "data:text/csv;charset=utf-8," + encodeURIComponent(csvRows.map(r => r.join(",")).join("\n"));
  const link = document.createElement("a");
  link.setAttribute("href", csvContent);
  const fileName = `MECIA_HACKS_3.0_Final_Round_Live_Leaderboard_${Date.now()}.csv`;
