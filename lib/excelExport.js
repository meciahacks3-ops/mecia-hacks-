 ...
 * Contains Master Attendance Sheet + Dedicated Sheets for each Time Slot and Judge Panel / Room
 */
export function exportAttendanceSheetExcel(teams = []) {
  const wb = XLSX.utils.book_new();
  const dateStr = new Date().toLocaleString();
  const students = extractAllStudentsRoster(teams);
  ...
  // 1. MASTER ATTENDANCE SHEET (All Students)
  const masterRows = [
    ['MECIA HACK 3.0 — OFFICIAL STUDENT PARTICIPANT ATTENDANCE & VERIFICATION SHEET'],
    ['Event:', 'Round 2 Prototype Demo & Final Evaluation', 'Date:', '31st August 2026', 'Generated On:', dateStr],
    ['Total Registered Teams:', teams.length, 'Total Student Participants:', students.length],
    [],
    attendanceHeaders
  ];
  ...
  students.forEach((s, idx) => {
    masterRows.push([
      ...
      s.phone,
      s.email,
      '', // Present/Absent blank for manual/evaluator marking
  ...
  wsMaster['!cols'] = [
    ...
    { wch: 28 }, // Student Signature
    { wch: 20 }  // Remarks
  ];
  XLSX.utils.book_append_sheet(wb, wsMaster, 'Master Attendance');

  // 2. DEDICATED SHEETS PER TIME SLOT
  const timeSlots = [
    { name: 'Slot 1 (09:30 AM)', filter: '09:30 AM - 11:30 AM' },
    { name: 'Slot 2 (12:15 PM)', filter: '12:15 PM - 02:15 PM' },
    { name: 'Slot 3 (02:30 PM)', filter: '02:30 PM - 04:15 PM' },
    { name: 'TBA Slot', filter: 'TBA' }
  ];

  timeSlots.forEach(slotInfo => {
    const slotStudents = students.filter(s => {
      if (slotInfo.filter === 'TBA') {
        return !s.timeSlot || s.timeSlot.toUpperCase() === 'TBA' || !['09:30 AM - 11:30 AM', '12:15 PM - 02:15 PM', '02:30 PM - 04:15 PM'].includes(s.timeSlot);
      }
      return s.timeSlot === slotInfo.filter;
    });

    if (slotStudents.length > 0) {
      const slotRows = [
        [`MECIA HACK 3.0 — ATTENDANCE SHEET: ${slotInfo.name.toUpperCase()}`],
        ['Presentation Window:', slotInfo.filter, 'Total Participants in Slot:', slotStudents.length],
        [],
        attendanceHeaders
      ];

      slotStudents.forEach((s, idx) => {
        slotRows.push([
          idx + 1,
          s.teamId,
          s.teamName,
      ...
      wsSlot['!cols'] = wsMaster['!cols'];
      XLSX.utils.book_append_sheet(wb, wsSlot, sanitizeSheetName(slotInfo.name));
    }
  });

  // 3. DEDICATED SHEETS PER JUDGE PANEL & LAB ROOM
  const knownPanels = Object.values(JUDGE_PROFILES);
  knownPanels.forEach(panel => {
    const panelStudents = students.filter(s => (s.assignedJudge || '').trim().toUpperCase() === panel.id.toUpperCase());
    if (panelStudents.length > 0) {
      const pRows = [
        [`MECIA HACK 3.0 — LAB ATTENDANCE SHEET: ${panel.id} (${panel.group})`],
        [`Lab Venue / Location: ${panel.location}`, `Faculty Evaluators: ${panel.namesText}`],
      ...
      wsPanel['!cols'] = wsMaster['!cols'];
      const sheetTitle = sanitizeSheetName(`${panel.id}_${panel.location.split(',')[1]?.trim() || panel.group}`);
      XLSX.utils.book_append_sheet(wb, wsPanel, sheetTitle);
    }
  });

  // 4. Download Workbook
  const filename = `Mecia_Hack_3.0_Student_Attendance_Sheet_${Date.now()}.xlsx`;
  XLSX.writeFile(wb, filename);
  return filename;
}
 ...
 * Supports modes:
 * - 'by-panel' (Default): Generates distinct sheets per judge panel/lab with page breaks for distributing to lab rooms
 * - 'all': Generates continuous master participant attendance roster
 * - 'by-slot': Generates distinct sheets per time slot with page breaks
 */
export function printAttendanceSheet(teams = [], options = {}) {
  if (typeof window === 'undefined') return;

  const mode = options.mode || 'by-panel'; // 'by-panel', 'all', 'by-slot'
  ...
  if (mode === 'by-panel') {
    ...
    // Also catch unassigned or custom panels
    const customOrUnassigned = [];

    students.forEach(s => {
      const jId = (s.assignedJudge || '').trim().toUpperCase();
      if (panelMap[jId]) {
        panelMap[jId].students.push(s);
      } else {
        customOrUnassigned.push(s);
      }
    });

    const activePanels = filterPanelId
      ? knownPanels.filter(p => p.id.toUpperCase() === filterPanelId)
      : knownPanels;

    bodyContent = activePanels.map((panel, pIdx) => {
      const panelStudents = panelMap[panel.id.toUpperCase()]?.students || [];
      ...
      const rowsHtml = sortedStudents.length === 0
        ? `<tr><td colspan="9" style="text-align:center; padding: 25px; font-style: italic; color: #555;">No student participants assigned to this judge panel yet.</td></tr>`
        : sortedStudents.map((s, idx) => `
            <tr style="${s.isLeader ? 'background-color: #fdfdf0;' : ''}">
              ...
              <td style="font-weight: bold; font-size: 12px;">
                ${s.studentName}
                ${s.phone && s.phone !== 'N/A' ? `<br/><small style="font-weight: normal; color: #666; font-size: 10px;">Ph: ${s.phone}</small>` : ''}
              </td>
              ...
              <td style="text-align: center; font-size: 10px; font-family: monospace; font-weight: bold;">
                ${s.timeSlot}
              </td>
              <td style="text-align: center; font-size: 11px; width: 85px;">
                <div style="display: flex; justify-content: center; gap: 8px; font-size: 10px; font-weight: bold;">
                  <span>[ &nbsp; ] P</span>
                  <span>[ &nbsp; ] A</span>
                </div>
              </td>
              <td style="width: 140px; text-align: center; vertical-align: bottom; padding-bottom: 6px;">
                <div style="border-bottom: 1.5px solid #222; width: 90%; margin: 0 auto; height: 18px;"></div>
      ...
      return `
        <div class="page-container" style="${pIdx < activePanels.length - 1 ? 'page-break-after: always;' : ''} margin-bottom: 30px;">
          <div class="header">
            ...
            <div style="display: grid; grid-template-columns: 1.5fr 1.5fr 1fr; gap: 10px; background: #f4f4f4; padding: 8px 12px; border: 1px solid #ccc; font-size: 12px;">
              <div><strong>🏛️ JUDGE PANEL:</strong> ${panel.id} (${panel.group})</div>
              <div><strong>📍 LAB VENUE / ROOM:</strong> <span style="font-weight: bold; background: #e0f2fe; padding: 2px 6px; border: 1px solid #93c5fd; border-radius: 3px;">${panel.location}</span></div>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th style="width: 140px;">Team ID & Name</th>
                <th style="width: 100px;">Role</th>
                <th style="width: 150px;">Student Name</th>
                <th style="width: 120px;">Enrollment / ID</th>
                <th style="width: 130px;">Department / Branch</th>
                <th style="width: 110px;">Time Slot</th>
                <th style="width: 85px;">Attendance</th>
                <th style="width: 140px;">Student Signature</th>
              </tr>
            </thead>
            <tbody>
              ${rowsHtml}
          ...
          <div style="margin-top: 18px; display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px; font-size: 11px; page-break-inside: avoid;">
    ...
    if (customOrUnassigned.length > 0 && !filterPanelId) {
      bodyContent += `
        <div class="page-container" style="page-break-before: always; margin-bottom: 30px;">
          <div class="header">
            <h1 style="margin: 0; font-size: 17px; text-transform: uppercase;">MECIA HACK 3.0 — ATTENDANCE SHEET: UNASSIGNED / CUSTOM</h1>
            <div style="margin-top: 4px; font-size: 12px;">Total Participants: ${customOrUnassigned.length}</div>
          </div>
          <table>
            <thead>
              <tr>
            ...
            <tbody>
              ${customOrUnassigned.map((s, idx) => `
                <tr>
                  <td style="text-align: center; font-weight: bold;">${idx + 1}</td>
                  <td><strong>${s.teamId}</strong><br/>${s.teamName}</td>
                  <td>${s.role}</td>
                  <td><strong>${s.studentName}</strong></td>
                  <td style="text-align: center; font-family: monospace; font-weight: bold;">${s.enrollmentNo}</td>
                  <td>${s.department}</td>
                  <td style="text-align: center; font-size: 10px;">${s.timeSlot}</td>
                  <td style="text-align: center;">[ &nbsp; ] P &nbsp; [ &nbsp; ] A</td>
                  <td style="text-align: center; vertical-align: bottom;"><div style="border-bottom: 1.5px solid #222; width: 90%; margin: 0 auto; height: 16px;"></div></td>
                </tr>
              `).join('')}
            </tbody>
          </table>
      ...
      <div class="page-container">
        <div class="header">
          <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 2px solid #000; padding-bottom: 6px; margin-bottom: 8px;">
            <div>
              <h1 style="margin: 0; font-size: 18px; text-transform: uppercase;">MECIA HACK 3.0 — MASTER STUDENT ATTENDANCE DIRECTORY</h1>
              <div style="font-size: 12px; font-weight: bold; color: #222; margin-top: 2px;">COMPLETE PARTICIPANTS SIGNATURE & VERIFICATION ROSTER</div>
            </div>
            <div style="text-align: right; font-size: 11px;">
              <div><strong>Date:</strong> 31st August 2026</div>
              <div><strong>Generated:</strong> ${dateStr}</div>
          ...
          <div style="display: flex; justify-content: space-between; font-size: 12px; background: #f4f4f4; padding: 6px 10px; border: 1px solid #ccc;">
            <div><strong>Total Registered Teams:</strong> ${teams.length}</div>
            <div><strong>Total Student Participants:</strong> ${sortedStudents.length}</div>
            <div><strong>Event Stage:</strong> Round 2 Prototype Evaluation</div>
          </div>
        </div>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th style="width: 120px;">Enrollment / ID</th>
              <th style="width: 130px;">Department / Branch</th>
              <th style="width: 110px;">Slot & Venue</th>
              <th style="width: 85px;">Attendance</th>
              <th style="width: 140px;">Student Signature</th>
            </tr>
          </thead>
            ...
            ${sortedStudents.map((s, idx) => `
              <tr style="${s.isLeader ? 'background-color: #fdfdf0;' : ''}">
                ...
                <td style="font-size: 10px; text-align: center;">
                  ...
                  <small style="color: #444;">${s.assignedJudge} (${s.panelLocation})</small>
                </td>
  ...
  const html = `
    ...
    <html>
    <head>
      <title>Mecia Hack 3.0 — Student Attendance & Signature Sheets</title>
      <style>
        @page {
          size: A4 landscape;
          margin: 8mm;
    ...
    <body>
      <div class="no-print" style="margin-bottom: 14px; display: flex; gap: 10px; align-items: center; background: #e0f2fe; padding: 10px 14px; border: 1px solid #93c5fd; border-radius: 6px;">
        <button onclick="window.print()" style="padding: 10px 22px; font-weight: bold; background: #107c41; color: #fff; border: none; border-radius: 4px; cursor: pointer; font-size: 13px;">
          🖨️ PRINT ATTENDANCE SHEETS (A4 LANDSCAPE)
        </button>
        ...
        <span style="font-size: 12px; color: #0369a1; margin-left: 10px;">
          💡 <em>Tip: Choose <strong>Landscape</strong> orientation and enable <strong>Background graphics</strong> in your printer settings for best results.</em>
        </span>
      </div>
...
/**
 * Exports CSV file with all student participants and signature column
 */
export function exportAttendanceCSV(teams = []) {
  const students = extractAllStudentsRoster(teams);
  const csvRows = [];

  csvRows.push(['Mecia Hack 3.0 — Official Student Attendance & Verification Sheet']);
  csvRows.push(['Generated On', new Date().toLocaleString()]);
  csvRows.push(['Total Registered Teams', teams.length]);
  csvRows.push(['Total Student Participants', students.length]);
  csvRows.push([]);
  ...
  const downloadAnchor = document.createElement('a');
  downloadAnchor.href = url;
  downloadAnchor.setAttribute('download', `Mecia_Hack_3.0_Student_Attendance_${Date.now()}.csv`);
  document.body.appendChild(downloadAnchor);
  downloadAnchor.click();
  document.body.removeChild(downloadAnchor);
}
