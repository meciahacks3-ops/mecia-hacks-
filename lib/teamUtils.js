...
export function computeFinalRoundScoreForTeam(team, evaluations = []) {
  ...
  // Direct sum of raw criteria (c1 + c2 + c3 + c4 + c5 out of 100 max) without percentage weightage
  const sumScore = extEvals.reduce((s, e) => {
    const rawSum = (Number(e.c1) || 0) + (Number(e.c2) || 0) + (Number(e.c3) || 0) + (Number(e.c4) || 0) + (Number(e.c5) || 0);
    const scoreVal = rawSum > 0 ? rawSum : (Number(e.totalScore) || 0);
    return s + scoreVal;
  }, 0);
