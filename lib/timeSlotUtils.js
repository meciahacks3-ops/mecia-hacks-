...
export async function saveTeamAssignment(supabaseClient, teamId, teamName, { assignedJudge, timeSlot, rawMainIdea }) {
  ...
  let cleanJudge = undefined;
  if (assignedJudge !== undefined) {
    const raw = String(assignedJudge).trim();
    if (!raw || raw.toLowerCase() === 'unassigned') {
      cleanJudge = 'Unassigned';
    } else {
      cleanJudge = raw.toUpperCase();
    }
    updatePayload.assigned_judge = cleanJudge;
  }

  let normSlot = undefined;
  let updatedMainIdea = rawMainIdea || '';
  if (timeSlot !== undefined) {
    normSlot = normalizeTimeSlot(timeSlot);
    updatedMainIdea = buildMainIdeaWithSlot(rawMainIdea, normSlot);
    updatePayload.main_idea = updatedMainIdea;
  }

  if (Object.keys(updatePayload).length === 0) {
    return { error: null, updatedMainIdea, normSlot, finalJudge: cleanJudge };
  }

  let query = supabaseClient.from('teams').update(updatePayload);
  if (teamId && String(teamId).length > 20) {
  ...
  // Safe fallback: if updating both main_idea and assigned_judge failed, attempt updating assigned_judge alone
  if (error && cleanJudge !== undefined && Object.keys(updatePayload).length > 1) {
    let fallbackQuery = supabaseClient.from('teams').update({ assigned_judge: cleanJudge });
    if (teamId && String(teamId).length > 20) {
  ...
  return { error, updatedMainIdea, normSlot, finalJudge: cleanJudge };
}
